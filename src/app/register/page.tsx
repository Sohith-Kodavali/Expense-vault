...
export default function RegisterPage() {
  const { register, isLoggedIn, loading } = useUser();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (!name || !password) { setError("Fill in all fields"); return; }
    if (password.length < 4) { setError("Password: min 4 characters"); return; }
    setSubmitting(true);
    const ok = await register(name, password);
    if (ok) router.push("/");
    else { setError("Name already taken"); setSubmitting(false); }
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-violet-50 via-white to-indigo-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950">
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1.5">Start tracking your expenses</p>
        </div>
        <div className="glass-card rounded-2xl p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            <Input label="Name" placeholder="Your name" value={name} onChange={(e) => setName(e.target.value)} />
            <Input label="Password" type="password" placeholder="Min. 4 characters" value={password} onChange={(e) => setPassword(e.target.value)} />
            ...
            <Button type="submit" loading={submitting} size="lg" className="w-full">Create Account</Button>
          </form>
        </div>
        <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-6">
          Already registered? <Link href="/login" className="font-semibold text-violet-600 hover:text-violet-700">Sign in</Link>
        </p>
      </motion.div>
    </div>
  );
